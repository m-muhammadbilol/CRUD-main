...
const eljsaddbutton = document.querySelector(".js-add-button");
let limit = 9;
let skip = 0;
...
function loader(bool) {
  ...
  if (bool) {
    Array.from({ length: 9 }, (_, index) => index).forEach(() => {
...
let elLightMode = document.querySelector(".light__mode");
let savedMode = localStorage.getItem("mode");

elDarkMode.addEventListener("click", () => {
  elHtml.setAttribute("data-theme", "dark");
  elDarkMode.classList.add("hidden");
  elLightMode.classList.remove("hidden");
  localStorage.setItem("mode", "dark");
});

elLightMode.addEventListener("click", () => {
  elHtml.setAttribute("data-theme", "light");
  elLightMode.classList.add("hidden");
  elDarkMode.classList.remove("hidden");
...
elCarForm.addEventListener("submit", (evt) => {
  ...
  formData.forEach((value, key) => {
    if (value.trim() == "") {
      ...
      const input = elCarForm.querySelector(`[name="${key}"]`);

      const toastDel = clone.firstElementChild;
...
eljsaddbutton.addEventListener("click", () => {
  const check = isLogin();
  if (check) {
