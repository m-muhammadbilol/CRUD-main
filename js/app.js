...
fetch(`https://json-api.uz/api/project/fn44-amaliyot/cars?limit=${limit}&skip=${skip}`)
  .then((res) => {
    return res.json();
  })
  .then((res) => {
...
function loader(bool) {
  ...
  if (bool) {
    Array.from({ length: 3 }, (_, index) => index).forEach(() => {
      elloader.appendChild(elTemplateSkeleton.cloneNode(true).content);
    });
  }
}
loader(true);
...
function deleteCars(id){
 fetch("https://json-api.uz/api/project/fn44-amaliyot/cars/"+ id , {method:"DELETE"} )
  .then((res) => {
    return res.text();
  })
  .then((res) => {
...
elContainer.addEventListener("click" , (evt)=>{
  if(evt.target.classList.contains('js-info')){
    evt.target.disabled = true
    const elPage = document.getElementById("infoHTML")
  }
}
)
